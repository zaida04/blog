...
For the sake of making these Dockerfiles not too long, I'll simplify the Dockerfile to only include the adapter_runner. The debug_adapter is a library crate that the adapter_runner uses, so it's not necessary to include it in the Dockerfile.
